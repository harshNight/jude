...
let advise = document.querySelector('#advise'); // input select in diagnosis box
    ...
    if (event.target.closest('button')) {
        ...
        if (symptomArea.value.length > 7) {
            ...
            if (data.sender == 'btn_diagnosis') {
                ...
                if (advise.value == 2) {
                    // displays the presciption box
                    final_box.classList.add('show-final');
                }
                if (advise.value == 1) {
                    // shoes the saving button ater clicking update in the diagnosis boc
                    event.target.innerHTML = 'saving';
                }
            }
            if (data.sender == 'btn_prescription') {
                //check if the lab status is 1
                // if so, report that the lab report must be set to 2 by the lab attendant
                // indicating that the text has een completed

                // if the status is either 0 or 2 meanding !=1
                // carry out the usual operation
            }
