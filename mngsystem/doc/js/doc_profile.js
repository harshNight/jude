...
const filterer = new RegExp(/^[A-Z]{2}-\d{4,6}-.+$/);
let span_appointment = document.querySelector('#span_patientID');
span_appointment.innerHTML = localStorage.getItem('appointment_id');
if (!span_appointment || !filterer.test(span_appointment.innerHTML)) {
    // return the user back to the index page;
}
else {
    let appointment_id = span_appointment.innerHTML;
    ...
    let promise = $.post('../api/profile_api.php', patient_details);
    promise.then(resolve => {
        
        if (resolve['result'] === true) {
            console.log(resolve);
            const td = Array.from(document.querySelectorAll('#profile_table td'));
        ...
        console.log(err.responseText + `found some errors`);
        // display error
        // log error
    });
}
